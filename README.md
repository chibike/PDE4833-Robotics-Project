...
Move Right Arm to a safe location

Move Left Arm to default pose

Calibrate Grippers

Wait for the search command
...
waits for the user to press the connect button
tries to connect with Baxter

waits for the user to press the detect button
asks Baxter to find the card tray

waits for Baxter to find the card tray
informs the user when the card tray has been found

waits for the user to select a card
waits for the user to press the fetch card's button

asks Baxter to fetch the selected card
...
Load camera scan positions

scan each position for aruco markers

calculate the 3d position of the marker relative to the robot's base

calculate the average 3d position for the all markers found

Use the markers to estimate the position of the card bin relative to the robot's base
...
fetch an image from Baxter's left-hand camera

locate the markers in the image

transform the marker positions from the camera's frame to the robot's frame

repeat until stopped

return marker positions found
